...
def _contract_pairs(top_contracts) -> list[tuple[str, int]]:
    """(contract, count) pairs from a stored or in-memory distribution.

    A stored record's ``top_contracts`` has been through
    ``firestore_store._firestore_safe``, which wraps each inner list as
    ``{"items": [...]}`` (Firestore forbids directly nested arrays); the
    local JSON pool keeps plain pairs and the forge passes tuples."""
    out = []
...
def _reach_share(pairs: list[tuple[str, int]], hero_i: int, want: str,
    ...
    *want*, counted over the layouts the hero's side declares.
    ...
    if not pairs or n_samples <= 0 or total < INVITE_COVER_MIN * n_samples:
        return None
    ours = [(k, c) for k, c in pairs if contract_side(k, hero_i) == 0]
    ...
    def dist(bid):
        ...
        if d is not None:
            return list(d.items()) if hasattr(d, "items") \
                else _contract_pairs(d)
        return _contract_pairs((rows.get(bid) or {}).get("top_contracts"))
...
def record_violations(rec: dict) -> tuple[list[str], list[str]]:
    ...
    check, and the invitation check against the record's own rollout. Lets
    the same gate vet historical pools and freshly forged batches alike.
    ...
    return (fatal
            + forcing_pass_violations(stem_entries, option_cards,
                                      dealer_i, hero_i)
            + invite_violations(option_cards, verdict.get("table") or [],
                                verdict.get("accepted") or "", hero_i,
                                int((rec.get("quality") or {}).get(
                                    "n_samples") or 0)), soft)
