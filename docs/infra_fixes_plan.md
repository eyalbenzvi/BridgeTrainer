...
## סטטוס ביצוע (עדכון)
- **T12 — דולג** לפי החלטת המשתמש: פיצול האינדקס לפי kind מונע חישוב מדויק של "ממתינות" ל-kind הלא-פעיל במסך הבית, ודורש מיגרציה מתואמת על ה-DB החי. T10 כבר צמצם דרסטית את קריאות האינדקס דרך ה-cache.
- **T2 — מבוצע** בגישה השמרנית: הקבועים `_CSS`/`_SHARED_JS` נשארים מקור-האמת ב-Python (כדי לא לשבור בדיקות מבוססות-קבוע), אך `write_app` פולט אותם כקבצי `app.css`/`bt-shared.js` חיצוניים ש-4 הדפים מקשרים — כך ה-HTML לכל דף מתכווץ מ-~90KB ל-KB בודדים והנכסים המשותפים נשמרים ב-cache בין ניווטים.
