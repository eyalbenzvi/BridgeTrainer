...
| `engine/scanner.py` thresholds | policy split (0.70/0.15), candidate floor 0.02 (raw softmax, not get_bid_candidates), stem-mass 0.05, depth caps | **[policy]** over Ben's numbers |
| `engine/maker.py` menu completion + `engine/verdict.py` divergence gate | rollout-implied candidates (COMPLETE_SHARE 0.25) added to the menu; policy-top refuted by its own rollout (> GAP_MAX) rejects the board | **[policy]** over Ben+DD numbers |
