...
## 6. The shipped architecture (final form on this branch)

The gate of §5 evolved into full **card-world grading** — the published
answer *changes* instead of the board dying, per the owner's direction.

**Vocabulary + DSL (engine/lead_gib_constraints, domain/constraints):**
GIB's language measured over the whole pool is a CLOSED vocabulary — 94
distinct clause patterns, 91,910 instances — and the table-driven compiler
covers 100% of it (fixture: tests/data/gib_vocab_patterns.json): lengths /
HCP / points; stops (full, partial, likely, negative "at best partial",
double); specific honor holdings (`!CKQ`, `no !DAK`, `Q+ in !D`) via the
`HonorSpec` primitive; `!SAKQ,no !S` via disjunction groups (`alt_groups`);
solid/exact/strong suit qualities; auction facts as recognised no-ops.
Promises the leader's own hand contradicts degrade gracefully and are
reported. No LLM anywhere — the vocabulary is template-generated by a rule
engine and finite by construction.

**Calibration (scripts/calibrate_gib_vocab.py):** every soft "announced X
but doesn't hold it" weight is a likelihood ratio MEASURED on real deals
(Laplace-smoothed, clamped), stored in `semantics/gib_calibration.json` and
auto-loaded. Measured on today's pool: stop 0.25, partial stop 0.32,
honor promises 0.02.

**Forward fix — the forge (engine/lead_maker._card_world_grade):** after
Ben's own gates pass, every would-be-published board now:
1. must have a deal that HONOURS its glosses' probabilistic promises
   (`gloss_violations`, same core-satisfaction test the grading assumes) —
   else `gloss_unfulfilled`;
2. is RE-GRADED on the compiled card constraints at calibrated weights
   (`card_world_evaluation` → a stock `LeadEvaluation`, so judge / metrics /
   record builder / notes run untouched, weighted layouts folded in by a
   paired weight-proportional resample); the interestingness gates re-run
   on the card-world values (`cards_<reason>` rejects);
3. must keep an overlapping answer set under the STRICT reading
   (`honor_sensitive` otherwise).
Ben still deals, bids, proposes candidates and supplies the softmax used by
the difficulty/trap model. Provenance is stamped in
`generator.grading.distribution` (`gib_cards_calibrated`, or
`ben_samples_fallback` when the auction constrained nothing recognisable —
an honest fallback, never a fake grade).

**Backward fix — the pool (scripts/migrate_lead_card_world.py):** four
phases over the 3,228 stored lead problems — (1) gloss-validate every deal
(contradicting deals are unfixable → delete); (2) RECALIBRATE on the
validated subset (p→1 by construction, so weights fit to the near-strict
clamp floor — the reading the validated pool justifies); (3) regrade every
survivor via `grade_record_card_world` (new verdict / by_mode / candidates /
notes / difficulty; the answer may change — that is the point; readings
that disagree → delete; unconstrained auctions → keep as-is); (4) with
`--apply`: merge updates, index-first deletes, write the recalibrated file,
then `regrade_attempts` so stored user history matches byte-for-byte.
**Default is DRY-RUN with a full JSON report and zero writes.**

Bootstrapping note (why the answers actually change): the pre-validation
pool honours announced stops only ~66-87% of the time, so calibration on it
yields soft weights and the reference board grades SA under the calibrated
reading. Phase 1 removes the gloss-contradicting deals; calibration on the
...
them; nothing compiles them yet).
